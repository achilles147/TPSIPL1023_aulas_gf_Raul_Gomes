...
desses carros faz com um litro de combustível.
...
def tabela():
    ...
    for i in range(5):
        print(f"""
        {i + 1} - {carros[i]}       - {kmlitro[i]} - {kmlitro[i] / 1000:.2f} - R$ {(kmlitro[i] / 1000) * 2.25:.2f}""")
    print(f"O menor consumo é do {carros[kmlitro.index(min(kmlitro))]}")

def floatinput(var):
    while True:
        try:
            user_input = float(input(var))
            return user_input
        except ValueError:
